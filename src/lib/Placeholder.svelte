...
<svg width="120%" height="120%" viewBox="0 -100 1171 241" fill="none">
	<defs>
		<filter id="noiseFilter_{uniqueId}" x="0%" y="0%" width="100%" height="100%">
			<feTurbulence baseFrequency="0.9" numOctaves="3" seed="5" stitchTiles="stitch" />
		</filter>

		<filter id="mainBlur_{uniqueId}" x="-20%" y="-50%" width="140%" height="200%">
	...
	<g filter="url(#mainBlur_{uniqueId})">
		<ellipse
			...
			opacity="0.8"
		/>
		<circle cx={colors[6].pos} cy="-20" r="180" fill="url(#grad2_{uniqueId})" opacity="0.7" />
		<ellipse
			cx={colors[7].pos}
			cy="10"
			rx="250"
